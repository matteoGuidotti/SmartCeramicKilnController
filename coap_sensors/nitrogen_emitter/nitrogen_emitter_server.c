...
#define SENSOR_TYPE "nitrogen_emitter"
...
PROCESS(ripening_notifier_server, "Server for nitrogen emitter");
AUTOSTART_PROCESSES(&nitrogen_emitter_server);
...
extern coap_resource_t nitrogen_emitter;
...
PROCESS_THREAD(nitrogen_emitter_server, ev, data)
{
	...
	/*while(!registered){
		...
		LOG_INFO("[NITROGEN EMITTER] sending registration message\n");	
	...
	LOG_INFO("Starting nitrogen emitter Server\n");
	
	coap_activate_resource(&nitrogen_emitter, "nitrogen_emitter");  
